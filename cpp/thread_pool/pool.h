...
class [[nodiscard]] thread_pool {
  public:
     ...
     * destroys all threads. Note that if the pool is paused, then any tasks
     * still in the queue will never be executed.
     */
    ~thread_pool() {
        wait_for_tasks();
        destroy_threads();
     ...
     * queue, or running in a thread. Note that get_tasks_total() ==
    ...
    parallelize_loop(const T1 first_index, const T2 index_after_last, F &&loop,
        ...
        if (blks.get_total_size() > 0) {
            ...
            for (size_t i = 0; i < blks.get_num_blocks(); ++i)
                mf[i] =
                    submit(std::forward<F>(loop), blks.start(i), blks.end(i));
            return mf;
        } else {
    ...
    template <typename F, typename T,
              typename R = std::invoke_result_t<std::decay_t<F>, T, T>>
    [[nodiscard]] multi_future<R>
    parallelize_loop(const T index_after_last, F &&loop,
                     const size_t num_blocks = 0) {
     ...
     * multi_future, so the user must use wait_for_tasks() or some other method
     * to ensure that the loop finishes executing, otherwise bad things will
     * happen.
     *
     * @tparam F The type of the function to loop through.
     * @tparam T1 The type of the first index in the loop. Should be a signed or
     ...
     * multi_future, so the user must use wait_for_tasks() or some other method
     * to ensure that the loop finishes executing, otherwise bad things will
     * happen. This overload is used for the special case where the first index
     * is 0.
     *
     * @tparam F The type of the function to loop through.
     * @tparam T The type of the loop indices. Should be a signed or unsigned
  ...
  private:
     ...
     *
     * @param thread_count_ The parameter passed to the constructor or reset().
    ...
    void worker() {
        while (running) {
            ...
            task_available_cv.wait(
                tasks_lock, [this] { return !tasks.empty() || !running; });
            if (running && !paused) {
    ...
    /**
     * @brief An atomic variable indicating whether the workers should pause.
     * When set to true, the workers temporarily stop retrieving new tasks out
     * of the queue, although any tasks already executed will keep running until
     * they are finished. When set to false again, the workers resume retrieving
     * tasks.
     */
...
class [[nodiscard]] synced_stream {
  public:
     ...
     * it. Should only be used if flushing is desired, otherwise '\n' should be
     * used instead.
     */
...
class [[nodiscard]] timer {
  public:
    ...
    /**
     * @brief Get the number of milliseconds that have elapsed between start()
     * and stop().
     *
